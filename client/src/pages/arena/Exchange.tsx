...
export default function ArenaExchange() {
    ...
    const fetchData = async () => {
        try {
            // 1. Pega os dados do gráfico (Candles)
            const chartRes = await api.get('/exchange/chart');
            setHistory(chartRes.data); // O controller já manda no formato {time, open, high, low, close}

            // 2. Pega o preço e supply atual
            const statsRes = await api.get('/exchange/stats');

            // Calculando o preço atual para exibir na tela baseada na sua fórmula
    ...
    const handleTrade = async (type: 'buy' | 'sell') => {
        if (!amount || parseFloat(amount) <= 0) return toast.error("Quantidade inválida");
        setLoading(true);
        try {
            // Chamando o seu executeTrade do controller
            await api.post('/exchange/trade', {
                action: type,
                amount: parseInt(amount)
            });

            toast.success("Ordem executada na Arena! 🏟️");
            setAmount('');
            await fetchData();
        } catch (err: any) {
