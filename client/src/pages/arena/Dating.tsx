...
import { Style, Mail } from '@mui/icons-material';
    ...
    return (
        <div className="pb-20 p-4 max-w-md mx-auto">
            <header className="flex justify-between items-center mb-4">
                ...
                {/* Abas Super Compactas */}
                <div className="bg-slate-900 p-1 rounded-xl border border-slate-800 flex">
                    <button 
                        onClick={() => setTab(0)}
                        className={`p-2 rounded-lg transition-all ${tab===0 ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500'}`}
                    >
                        <Style fontSize="small"/>
                    </button>
                    <button 
                        onClick={() => setTab(1)}
                        className={`p-2 rounded-lg transition-all ${tab===1 ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-500'}`}
                    >
                        <Mail fontSize="small"/>
                    </button>
                </div>
